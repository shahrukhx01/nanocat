...
from nanocat.audio.vad.vad_analyzer import VADAnalyzer, VADParams
...
class SileroOnnxModel:
    ...
    def __call__(self, x, sr: int):
        ...
        if np.shape(x)[-1] != num_samples:
            raise ValueError(
                f"Provided number of samples is {np.shape(x)[-1]} (Supported values: 256 for 8000 sample rate, 512 for 16000)"  # noqa: E501
            )
...
class SileroVADAnalyzer(VADAnalyzer):
    def __init__(
        self, *, sample_rate: Optional[int] = None, params: VADParams = VADParams()
    ):
