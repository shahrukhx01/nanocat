...
from nanocat.services.ai_service import AIService
...
class LLMService(AIService):
        ...
        *,
        ...
        if start_callback:
            ...
            with warnings.catch_warnings():
                ...
                warnings.warn(
                    "Parameter 'start_callback' is deprecated, just put your code on top of the actual function call instead.",  # noqa: E501
                    DeprecationWarning,
                )
    ...
    async def call_function(
        ...
        *,
        ...
        if (
            function_name not in self._functions.keys()
            and None not in self._functions.keys()
        ):
            return

        task = self.create_task(
            self._run_function_call(
    ...
    async def _run_function_call(
        ...
        if len(signature.parameters) > 1:
            ...
            with warnings.catch_warnings():
                warnings.simplefilter("always")
                warnings.warn(
                    "Function calls with parameters `(function_name, tool_call_id, arguments, llm, context, result_callback)` are deprecated, use a single `FunctionCallParams` parameter instead.",  # noqa: E501
                    DeprecationWarning,
                )
    ...
    async def _cancel_function_call(self, function_name: str):
        ...
        for task, tool_call_id, name in self._function_call_tasks:
            if name == function_name:
                ...
                logger.debug(
                    f"{self} Cancelling function call [{name}:{tool_call_id}]..."
                )
                ...
                logger.debug(
                    f"{self} Function call [{name}:{tool_call_id}] has been cancelled"
                )
    ...
    def _function_call_task_finished(self, task: asyncio.Task):
        tuple_to_remove = next(
            (t for t in self._function_call_tasks if t[0] == task), None
        )
        if tuple_to_remove:
            ...
            asyncio.run_coroutine_threadsafe(
                self.wait_for_task(task), self.get_event_loop()
            )
