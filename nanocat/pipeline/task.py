...
from nanocat.pipeline.task_observer import TaskObserver
...
class PipelineTask(BaseTask):
    ...
    def __init__(
        ...
        *,
        ...
        self._observer = TaskObserver(
            observers=observers, task_manager=self._task_manager
        )
    ...
    async def _heartbeat_push_handler(self):
        ...
        while True:
            ...
            await self._source.queue_frame(
                HeartbeatFrame(timestamp=self._clock.get_time())
            )
        ...
        while True:
            try:
                frame = await asyncio.wait_for(
                    self._heartbeat_queue.get(), timeout=wait_time
                )
                process_time = (
                    self._clock.get_time() - frame.timestamp
                ) / 1_000_000_000
                logger.trace(
                    f"{self}: heartbeat frame processed in {process_time} seconds"
                )
    ...
    async def _idle_monitor_handler(self):
        ...
        while running:
            try:
                ...
                if isinstance(frame, StartFrame) or isinstance(
                    frame, self._idle_timeout_frames
                ):
    ...
    async def _idle_timeout_detected(self) -> bool:
        ...
        if self._cancel_on_idle_timeout:
            logger.warning("Idle pipeline detected, cancelling pipeline task...")
            await self.cancel()
            return False
        return True
